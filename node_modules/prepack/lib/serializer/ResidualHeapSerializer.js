...
class ResidualHeapSerializer {
  constructor(realm, logger, modules, residualHeapValueIdentifiers, residualHeapInspector, residualValues, residualFunctionInstances, residualClassMethodInstances, residualFunctionInfos, options, referencedDeclaredValues, additionalFunctionValuesAndEffects, additionalFunctionValueInfos, declarativeEnvironmentRecordsBindings, react, referentializer, generatorDAG, conditionalFeasibility, additionalGeneratorRoots) {
    ...
    this.residualFunctions = new _ResidualFunctions.ResidualFunctions(this.realm, options, this.modules, this.requireReturns, {
      ...
      createFunction: (containingAdditionalFunction, statements) => {
        ...
        this._getPrelude(containingAdditionalFunction).push(t.functionDeclaration(id, [], t.blockStatement(statements)));
        return id;
      }
    ...
    this.emitter = new _Emitter.Emitter(this.residualFunctions, referencedDeclaredValues, conditionalFeasibility, this.preludeGenerator.derivedIds);
  ...
  _emitProperty(val, key, desc, deleteIfMightHaveBeenDeleted = false) {
    ...
    let locationFunction = () => {
      let serializedKey = key instanceof _index2.SymbolValue || key instanceof _index2.AbstractValue ? this.serializeValue(key) : this.generator.getAsPropertyNameExpression(key);
  ...
  emitDefinePropertyBody(deleteIfMightHaveBeenDeleted, locationFunction, val, key, desc) {
    ...
    let serializedKey = key instanceof _index2.SymbolValue || key instanceof _index2.AbstractValue ? this.serializeValue(key) : this.generator.getAsPropertyNameExpression(key, /*canBeIdentifier*/false);
  ...
  _serializeDeclarativeEnvironmentRecordBinding(residualFunctionBinding) {
    if (!residualFunctionBinding.serializedValue) {
      ...
      residualFunctionBinding.serializedValue = value !== undefined ? this.serializeValue(value) : _internalizer.voidExpression;
  ...
  _getTarget(val, trace) {
    ...
    if (generators.length === 0) {
      ...
      if (this._options.delayInitializations && (referencingOnlyAdditionalFunction === undefined || !functionValues.includes(referencingOnlyAdditionalFunction))) {
    ...
    this.emitter.dependenciesVisitor(val, {
      onArrayWithWidenedNumericProperty: dependency => {
        ...
        (0, _invariant2.default)(referencingOnlyAdditionalFunction === undefined || this.emitter.emittingToAdditionalFunction(), "additional function inconsistency");
  ...
  _getPrelude(additionalFunction) {
  ...
  _declare(emittingToResidualFunction, referencingOnlyAdditionalFunction, bindingType, id, init) {
  ...
  serializeValue(val, referenceOnly, bindingType) {
    ...
    if (this.residualHeapValueIdentifiers.needsIdentifier(val)) {
      if (init) {
        ...
        if (init !== id) {
          this._declare(!!target.usedOnlyByResidualFunctions, target.referencingOnlyAdditionalFunction, bindingType || "var", id, init);
        }
    ...
    if (lenProperty instanceof _index2.AbstractValue || _singletons.To.ToLength(realm, lenProperty) !== numberOfIndexProperties) {
      if (!(lenProperty instanceof _index2.AbstractValue) || lenProperty.kind !== "widened property") {
        ...
        this.emitter.emitNowOrAfterWaitingForDependencies([val, lenProperty], () => {
          this._assignProperty(() => t.memberExpression(this.getSerializeObjectIdentifier(val), t.identifier("length")), () => this.serializeValue(lenProperty), false /*mightHaveBeenDeleted*/
          );
          if (semaphore !== undefined) semaphore.releaseOne();
        }, this.emitter.getBody());
      }
    ...
    if (val.temporalAlias !== undefined) {
      ...
      for (let [key, propertyBinding] of val.properties) {
        ...
        if (this._canEmbedProperty(val, key, descriptor)) {
          ...
          let serializedValue = _internalizer.voidExpression;
          if (delayReason) {
          ...
          dummyProperties.add(key);
          let serializedKey = this.generator.getAsPropertyNameExpression(key);
          props.push(t.objectProperty(serializedKey, _internalizer.voidExpression));
        }
      }
    }
    this._emitObjectProperties(val, remainingProperties,
  ...
  _serializeValueObjectViaConstructor(val, skipPrototype, classConstructor) {
    ...
    if (val.temporalAlias === undefined) {
      ...
      return t.sequenceExpression([t.assignmentExpression("=", t.memberExpression(_internalizer.constructorExpression, t.identifier("prototype")), classConstructor ? t.memberExpression(serializedProto, t.identifier("prototype")) : serializedProto), t.newExpression(_internalizer.constructorExpression, [])]);
    } else {
  ...
  _serializeAbstractValueHelper(val) {
    ...
    if (serializedValue.type === "Identifier") {
      ...
      (0, _invariant2.default)(!this.preludeGenerator.derivedIds.has(id.name) || this.emitter.cannotDeclare() || this.emitter.hasBeenDeclared(val) || this.emitter.emittingToAdditionalFunction() && this.referencedDeclaredValues.get(val) === undefined, `an abstract value with an identifier "${id.name}" was referenced before being declared`);
    }
  ...
  _serializeGlobalBinding(boundName, binding) {
    ...
    if (!binding.serializedValue) {
      ...
      if (boundName === "undefined") {
        binding.serializedValue = _internalizer.voidExpression;
  ...
  serialize() {
    ...
    this.getStatistics().generators++;
    (0, _invariant2.default)(this.emitter.declaredCount() <= this.preludeGenerator.derivedIds.size);
