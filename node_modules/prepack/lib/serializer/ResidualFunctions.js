...
class ResidualFunctions {
  ...
  _createFunctionExpression(params, body) {
  ...
  spliceFunctions(rewrittenAdditionalFunctions) {
    ...
    for (let [funcValue, additionalFunctionInfo] of this.additionalFunctionValueInfos.entries()) {
      ...
      if (funcValue instanceof _index.ECMAScriptSourceFunctionValue && funcValue.$Strict) {
    ...
    for (let [funcBody, instances] of functionEntries) {
      ...
      let naiveProcessInstances = instancesToSplice => {
        ...
        for (let instance of instancesToSplice) {
          ...
          if (this.residualClassMethodInstances.has(functionValue)) {
            ...
            funcOrClassNode = new _ResidualFunctionInstantiator.ResidualFunctionInstantiator(factoryFunctionInfos, this._getIdentifierReplacements(funcBody, residualFunctionBindings), this._getCallReplacements(funcBody), this._createFunctionExpression(params, funcBody)).instantiate();
            ...
            if (scopeInitialization.length > 0) funcOrClassNode.body.body = scopeInitialization.concat(funcOrClassNode.body.body);
          }
          let id = this.locationService.getLocation(functionValue);
          (0, _invariant2.default)(id !== undefined);
      ...
      if (normalInstances.length > 0 && !this._shouldUseFactoryFunction(funcBody, normalInstances)) {
        ...
        let factoryNode = new _ResidualFunctionInstantiator.ResidualFunctionInstantiator(factoryFunctionInfos, this._getIdentifierReplacements(funcBody, sameResidualBindings), this._getCallReplacements(funcBody), this._createFunctionExpression(factoryParams, funcBody)).instantiate();
        ...
        for (let instance of normalInstances) {
          ...
          this.residualFunctionInitializers.hasInitializerStatement(functionValue) || usesThis || hasFunctionArg || firstUsage !== undefined && !firstUsage.isNotEarlierThan(insertionPoint) || this.functionPrototypes.get(functionValue) !== undefined) {
