...
class ModuleTracer extends _realm.Tracer {
  ...
  _callRequireAndAccelerate(isTopLevelRequire, moduleIdValue, performCall) {
    ...
    do {
      ...
      if (isTopLevelRequire && effects !== undefined && !(effects.result instanceof _completions.AbruptCompletion)) {
        ...
        for (let nestedModuleId of this.uninitializedModuleIdsRequiredInEvaluateForEffects) {
          ...
          if (this.modules.accelerateUnsupportedRequires && nestedEffects !== undefined && nestedEffects.result instanceof _index2.Value && this.modules.isModuleInitialized(nestedModuleId)) {
    ...
    if (this.evaluateForEffectsNesting > 0) {
      ...
      return downgradeErrorsToWarnings(realm, () => {
        ...
        try {
          ...
          if (effects === undefined || effects.result instanceof _completions.AbruptCompletion) {
...
class Modules {
  constructor(realm, logger, logModules, delayUnsupportedRequires, accelerateUnsupportedRequires) {
    this.realm = realm;
    this.logger = logger;
  ...
  isModuleInitialized(moduleId) {
    ...
    try {
      ...
      // for lint unused
      (0, _invariant2.default)(modifiedBindings);

      if (result instanceof _completions.AbruptCompletion) return undefined;
      (0, _invariant2.default)(result instanceof _index2.Value);

      if (!generator.empty() || result instanceof _index2.ObjectValue && createdObjects.has(result)) return undefined;
