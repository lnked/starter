...
function createAbstract(realm, typeNameOrTemplate, name, ...additionalValues) {
  ...
  if (!name) {
    ...
    let kind = _index.AbstractValue.makeKind("abstractCounted", (realm.objectCount++).toString()); // need not be an object, but must be unique
    result = _index.AbstractValue.createFromTemplate(realm, throwTemplate, type, [locVal], kind);
  } else {
    let kind = _index.AbstractValue.makeKind("abstract", name);
