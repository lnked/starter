...
class AbstractObjectValue extends _index.AbstractValue {
  ...
  $GetPrototypeOf() {
    ...
    if (elements.size === 1) {
      ...
      let joinedObject = _singletons.Join.joinValuesAsConditional(realm, cond, p1, p2);
      ...
      let result = _index.AbstractValue.createFromBuildFunction(realm, _index.ObjectValue, [primitiveValue], ([p]) => {
        ...
        return realm.isCompatibleWith(realm.MOBILE_JSC_VERSION) || realm.isCompatibleWith("mobile") ? t.memberExpression(p, _internalizer.protoExpression) : t.callExpression(getPrototypeOf, [p]);
      });
      ...
      for (let cv of elements) {
        ...
        if (joinedObject === undefined) {
          ...
          joinedObject = _singletons.Join.joinValuesAsConditional(realm, cond, p, joinedObject);
        }
      }
      (0, _invariant2.default)(joinedObject instanceof AbstractObjectValue);
      return joinedObject;
  ...
  $GetPartial(P, Receiver) {
    ...
    if (this.values.isTop()) {
      ...
      if (this.$Realm.isInPureScope()) {
        ...
        return _index.AbstractValue.createTemporalFromBuildFunction(this.$Realm, _index.Value, [Receiver, P], ([o, p]) => t.memberExpression(o, p, true));
      }
      let error = new _errors.CompilerDiagnostic("property access on unknown object", this.$Realm.currentLocation, "PP0031", "FatalError");
      this.$Realm.handleError(error);
      throw new _errors.FatalError();
      ...
      if (this.$Realm.isInPureScope()) {
        ...
        this.$Realm.evaluateWithPossibleThrowCompletion(() => {
          ...
          if (typeof P === "string") {
            ...
            generator.emitStatement([Receiver, V], ([objectNode, valueNode]) => t.expressionStatement(t.assignmentExpression("=", t.memberExpression(objectNode, propName, !t.isIdentifier(propName)), valueNode)));
          } else {
            // Coercion can only have effects on anything reachable from the key.
            _singletons.Havoc.value(this.$Realm, P);
