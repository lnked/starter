...
function construct_empty_effects(realm) {
  return new Effects(realm.intrinsics.empty, new _generator.Generator(realm, "construct_empty_effects"), new Map(), new Map(), new Set());
}
...
class Realm {
  ...
  evaluateWithUndo(f, defaultValue = this.intrinsics.undefined) {
    ...
    try {
      ...
      return effects.result instanceof _index.Value ? effects.result : defaultValue;
    } finally {
  ...
  evaluateForFixpointEffects(iteration) {
    try {
      ...
      let f = () => {
        ...
        (0, _invariant2.default)(result instanceof _completions.Completion || result instanceof _index.Value);
        return result;
      };
  ...
  evaluateWithAbstractConditional(condValue, consequentEffectsFunc, alternateEffectsFunc) {
    ...
    if (effects1 === undefined || effects2 === undefined) {
      ...
      joinedEffects = _singletons.Join.joinEffects(this, condValue, effects1, effects2);
      completion = joinedEffects.result;
  ...
  _emitPropertAssignments(gen, pbindings, newlyCreatedObjects) {
    ...
    pbindings.forEach((val, key, map) => {
      if (key.object instanceof _index.ObjectValue && (newlyCreatedObjects.has(key.object) || key.object.refuseSerialization)) {
        return;
      }
  ...
  captureEffects(completion) {
    ...
    completion.savedEffects = new Effects(this.intrinsics.undefined, this.generator, this.modifiedBindings, this.modifiedProperties, this.createdObjects);
  ...
  createErrorThrowCompletion(type, message) {
    ...
    (0, _invariant2.default)(message instanceof _index.StringValue);
    this.nextContextLocation = this.currentLocation;
    return new _completions.ThrowCompletion(this, (0, _index2.Construct)(this, type, [message]), this.currentLocation);
  }
