...
function handleNestedOptimizedFunctions(realm, reconciler, staticMarkup) {
  for (let _ref of reconciler.nestedOptimizedClosures) {
    let { func, evaluatedNode, componentType, context, branchState } = _ref;
    ...
    let closureEffects = reconciler.resolveNestedOptimizedClosure(func, [], componentType, context, branchState, evaluatedNode);
    ...
    let closureEffectsRenderedToString = realm.evaluateForEffectsWithPriorEffects([closureEffects], () => {
      ...
      (0, _invariant2.default)(closureEffects.result instanceof _index.Value);
...
function renderToString(realm, reactElement, staticMarkup) {
  ...
  let reconciler = new _reconcilation.Reconciler(realm, { firstRenderOnly: true }, reactStatistics);
  ...
  (0, _invariant2.default)(effects.result instanceof _index.Value);
  let serverRenderer = new ReactDOMServerRenderer(realm, staticMarkup);
