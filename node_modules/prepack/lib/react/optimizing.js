...
function applyWriteEffectsForOptimizedComponent(realm, componentType, effects, componentTreeState, evaluatedNode, writeEffects, environmentRecordIdAfterGlobalCode) {
...
function optimizeReactNestedClosures(realm, reconciler, writeEffects, environmentRecordIdAfterGlobalCode, logger) {
  ...
  for (let _ref2 of reconciler.nestedOptimizedClosures) {
    ...
    let closureEffects = reconciler.resolveNestedOptimizedClosure(func, nestedEffects, componentType, context, branchState, evaluatedNode);
    if (realm.react.verbose) {
...
function optimizeReactComponentTreeRoot(realm, componentRoot, config, writeEffects, environmentRecordIdAfterGlobalCode, logger, statistics, reactSerializerState) {
  let reconciler = new _reconcilation.Reconciler(realm, config, statistics, logger, reactSerializerState);
