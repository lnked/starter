...
exports.default = function (ast, strictCode, env, realm) {
  ...
  function composeNestedThrowEffectsWithHandler(c, priorEffects = []) {
    ...
    if (consequent instanceof _completions.JoinedAbruptCompletions || consequent instanceof _completions.PossiblyNormalCompletion) {
      ...
      consequentEffects = realm.evaluateForEffectsWithPriorEffects(priorEffects, () => {
      ...
      c.consequentEffects.result = c.consequent = new _completions.AbruptCompletion(realm.intrinsics.empty);
    }
    priorEffects.pop();
    ...
    if (alternate instanceof _completions.PossiblyNormalCompletion || alternate instanceof _completions.JoinedAbruptCompletions) {
      ...
      alternateEffects = realm.evaluateForEffectsWithPriorEffects(priorEffects, () => {
        (0, _invariant2.default)(ast.handler);
        return env.evaluateCompletionDeref(ast.handler, strictCode, alternate);
      }, "composeNestedThrowEffectsWithHandler/2");
      c.alternateEffects.result = c.alternate = new _completions.AbruptCompletion(realm.intrinsics.empty);
    }
    priorEffects.pop();
  ...
  function composeNestedEffectsWithFinalizer(c, priorEffects = []) {
    ...
    if (consequent instanceof _completions.JoinedAbruptCompletions || consequent instanceof _completions.PossiblyNormalCompletion) {
      ...
      if (!(consequentEffects.result instanceof _completions.AbruptCompletion)) consequentEffects.result = consequent;
    }
    priorEffects.pop();
    let alternate = c.alternate;
    let alternateEffects = c.alternateEffects;
    ...
    if (alternate instanceof _completions.PossiblyNormalCompletion || alternate instanceof _completions.JoinedAbruptCompletions) {
      ...
      if (!(alternateEffects.result instanceof _completions.AbruptCompletion)) alternateEffects.result = alternate;
    }
    priorEffects.pop();
    return _singletons.Join.joinEffects(realm, c.joinCondition, consequentEffects, alternateEffects);
  }
